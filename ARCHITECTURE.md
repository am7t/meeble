...
Profile, post, comment, reaction, and follow tables have initial constraints and indexes. The profile editor writes the signed-in user's own record through a server-side form. Feed endpoints bind writes to the signed-in user and enforce post visibility and ownership server-side. Follow/unfollow actions and public profile pages remain future work.
